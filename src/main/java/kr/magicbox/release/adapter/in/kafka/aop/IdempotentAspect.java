package kr.magicbox.release.adapter.in.kafka.aop;

import kr.magicbox.release.adapter.in.kafka.event.InboxEvent;
import kr.magicbox.release.adapter.in.kafka.properties.InboxProperties;
import kr.magicbox.release.adapter.out.persistence.entity.ReleaseInboxEntity;
import kr.magicbox.release.adapter.out.persistence.entity.ReleaseInboxStatus;
...
public class IdempotentAspect {
    ...
    @Around("@annotation(kr.magicbox.release.adapter.in.kafka.annotation.Idempotent)")
    public Object around(ProceedingJoinPoint pjp) {
        ...
        if (isTooOld(occurredAt)) {
            ...
            transactionTemplate.executeWithoutResult(status ->
                releaseInboxRepository.save(ReleaseInboxEntity.builder()
                        .kafkaKey(kafkaKey)
                        .topic(consumerRecord.topic())
                        .partition(consumerRecord.partition())
                        .offset(consumerRecord.offset())
                        .status(ReleaseInboxStatus.DEAD_LETTERED)
        ...
        return transactionTemplate.execute(status -> {
            if (releaseInboxRepository.existsByKafkaKey(kafkaKey)) {
                log.warn("[Inbox] 중복 메시지 폐기. kafkaKey={}", kafkaKey);
                return null;
            }
            ReleaseInboxEntity inbox = releaseInboxRepository.save(ReleaseInboxEntity.builder()
                    .kafkaKey(kafkaKey)
                    .topic(consumerRecord.topic())
                    .partition(consumerRecord.partition())
                    .offset(consumerRecord.offset())
                    .status(ReleaseInboxStatus.PENDING)
                    .occurredAt(occurredAt)
                    .build());
            try {
                pjp.proceed();
